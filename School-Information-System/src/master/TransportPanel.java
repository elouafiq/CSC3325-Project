...
public class TransportPanel extends JFrame {
  ...
  void updateTable(){
     ...
     for (int i = listOfVehicles.size() - 1; i  >= 0; i--){
       ...
       data[listOfVehicles.size() - 1 - i][1] = temp.matricule;
 ...
 public void Add() {
   /* if ( dayTextField.getText().equals("") || monthTextField.getText().equals("") ||
     ...
     GregorianCalendar date = new GregorianCalendar(Integer.parseInt(
          yearTextField.getText()), Integer.parseInt(monthTextField.getText()),
...
String path = "data.txt";
  ...
  for(int i = 0; i<listOfVehicles.size();i++){
    out.writeObject(listOfVehicles.get(i));
  }
...
while(fis.available() > 0)
  listOfVehicles.add((Vehicle) in.readObject());
